...
class Dreamer(tools.Module):
  ...
  def _train(self, data, log_images):
    with tf.GradientTape() as model_tape:
      ...
      #print(physics_pred.mean().shape)
      #tf.print(physics_pred.mean())
      #tf.print(data['physics'])
      #print(likes.values)
      #print(data['physics'].shape)
      #tf.print(tf.sqrt(tf.reduce_mean(tf.square(data['physics'] - physics_pred.mode()),axis=(0,1))))
      #tf.print(tf.sqrt(tf.reduce_mean(tf.square(data['physics'] - physics_pred.mean()),axis=(0,1))))
      #print('I dont')
      phys_error = tf.sqrt(tf.reduce_mean(tf.square(tf.subtract(physics_pred, data['physics'])),axis=(0,1)))
      if self._c.pcont:
      ...
      model_loss = self._c.kl_scale * div - sum(likes.values()) + phy_loss
  ...
  def _build_model(self):
    ...
    self._physics = models.DenseDecoderDet((), 2, self._c.num_units,
            act=tf.nn.swish)
    if self._c.pcont:
  ...
  def _scalar_summaries(
    ...
    self._metrics['linear_vel_err'].update_state(phys_error)
